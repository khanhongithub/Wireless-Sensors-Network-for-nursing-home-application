...
PROCESS_THREAD(gateway_main_process, ev, data) {
	...
	*/
	...
	broadcast_open(&broadcastConn, 149, &broadcast_callbacks);
...
static void broadcast_recv(struct broadcast_conn *c, const linkaddr_t *from) {
	...
	int rxBytes = 0;
		...
		if (rxBytes>0) {
            //printf("\r\n%d %d",packet.node_list[0],packet.node_status[2]);
	    	...
	    	snprintf(inter, sizeof(inter), "%d", from->u8[1]);
	    	strcat(head, inter);
	    	strcat(head, " ");
